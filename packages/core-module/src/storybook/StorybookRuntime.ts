...
import { AppRouterStore, createAppRouterStore, FireflyRuntime, FireflyRuntimeOptions, getMswPlugin, IFireflyRuntime, MswPluginName, ReactRouterRuntime, ReactRouterRuntimeScope, RegisterRequestHandlersOptions } from "@squide/firefly";
import { Logger } from "@workleap/logging";
import type { RequestHandler } from "msw";

// eslint-disable-next-line @typescript-eslint/no-explicit-any
export class StorybookRuntime<TRuntime extends FireflyRuntime = any> extends ReactRouterRuntime<TRuntime> implements IFireflyRuntime {
    ...
    // Temporary until it's moved to Squide
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    get honeycombInstrumentationClient(): any {
        throw new Error("Method not implemented.");
    }

    startScope(logger: Logger): TRuntime {
        return (new StorybookRuntimeScope(this, logger) as unknown) as TRuntime;
    }
}

export class StorybookRuntimeScope<TRuntime extends StorybookRuntime = StorybookRuntime> extends ReactRouterRuntimeScope<TRuntime> implements IFireflyRuntime {
    getMswState() {
